...
export const fetchRSS = (url, options) =>
  fetch(url, {
    method: "GET",
    headers: {
  ...
  return new Promise((accept, reject) => {
    fetchRSS(url, { signal }).then((res) => {
      ...
      if (contentType && contentType.indexOf("text/html") > -1) {
        ...
        cr.abort();
        fetchRSS(url + "?format=xml").then((fxml) => {
...
export const cleanXML = (json) => {
  ...
  if (Array.isArray(json.rss.channel)) channel = json.rss.channel[0];
